...
기존 AWS native 솔루션 이용중 한계를 느껴 ECS, EKS로 마이그레이션.
 1. 잦은 배포가 있다. EC2 안에서 docker container로 구동 중에 여러 문제점을 느겼다 (어떤 문제점???)
2. 웹과 앱을 동시에 개발한다. 웹은 react, 앱은 react native로 개발중임. 
3. 잦은 릴리즈와 MSA 형식 도입을 위해 EKS 전환 고려.
4. **AI모듈**을 접목해 가계부 서비스만의 AI로 편의성과, 소비 패턴 분석 및 추천.
